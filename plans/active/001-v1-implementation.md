...
- **Post-v1 — non-parseable memos sort by file mtime, not filename.** Original M2 decision used lexicographic filename sort for fallback memos. A batch of converted `.qta` files (`Carkeek Park 4.m4a`, `10.m4a`, etc.) surfaced the bug: `"10"` < `"4"` in string comparison, so the markdown rendered in the wrong order. New `_file_sort_key` uses `path.stat().st_mtime` for tier 1 (non-parseable). Works for the common case (Shortcut-recorded memos sync to iCloud individually, so mtime = recording order). Limitation: batch-converted files share near-identical mtimes that reflect ffmpeg's processing order, not recording order — unrecoverable without filename metadata. Accepted as a legacy-only edge case.
- **Post-v1 — `render_markdown` trusts caller order.** Moving the non-parseable sort to mtime means the sort key needs filesystem access (`path.stat()`), which doesn't belong in a pure renderer that only sees `Memo` objects. `render_markdown` no longer sorts; the caller (`process_new_memos`) sorts at the filesystem boundary via `_file_sort_key`, and `regenerate_run` reads the already-ordered `state["runs"][run_id]["files"]` list. `_memo_sort_key` deleted (was only used inside `render_markdown`). Test `test_render_markdown_sorts_chronologically` renamed and rewritten to `test_render_markdown_preserves_input_order`.
- **Post-v1 — `transcribe_file` strips whitespace at the Groq boundary.** Whisper's BPE tokenizer prepends a space to most decoded tokens, so the raw API response nearly always starts with `" "`. Stripping at the ingress point keeps stored transcriptions clean without defensive stripping at render time. New test `test_transcribe_file_strips_leading_whitespace` pins the behavior. Existing state was refreshed via `--all` to apply the strip to previously-stored transcriptions.
- **Post-v1 — smoke fixtures expanded to cover the fallback heading path.** Added two non-parseable fixtures (`Carkeek Park 10.m4a`, `Carkeek Park 11.m4a`) alongside the existing parseable pair. Smoke driver now copies all fixtures into a temp `memo_dir`, pins deterministic mtimes on the non-parseable ones via `os.utime`, and asserts: four memos total, parseable ones clustering first with `## Memo N — HH:MM — LAT, LONG`, non-parseable ones following with fallback headings in mtime order, and no transcription line starting with a space (the whitespace-strip invariant). Trade-off: +2 Groq calls per smoke run (2 → 4), +823 KB in the repo.
